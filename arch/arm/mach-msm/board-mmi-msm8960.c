...
static struct pm8xxx_gpio_init pm8921_gpios_teufel[] __initdata = {
...
static struct pm8xxx_gpio_init pm8921_gpios_vanquish[] __initdata = {
	PM8XXX_GPIO_DISABLE(6),				 /* Disable unused */
	PM8XXX_GPIO_DISABLE(7),				 /* Disable NFC */
	PM8XXX_GPIO_INPUT(16,	    PM_GPIO_PULL_UP_30), /* SD_CARD_WP */
...
static struct pm8xxx_gpio_init *pm8921_gpios_ptr __initdata = pm8921_gpios_vanquish;
static int gpio_init_table_size  __initdata = ARRAY_SIZE(pm8921_gpios_vanquish);
...
static void __init pm8921_gpio_mpp_init(void)
{
	...
	if(machine_is_teufel()){
		pm8921_gpios_ptr= pm8921_gpios_teufel;
		gpio_init_table_size  = ARRAY_SIZE(pm8921_gpios_teufel);
	}

	for (i = 0; i < gpio_init_table_size; i++) {
		rc = pm8xxx_gpio_config(pm8921_gpios_ptr[i].gpio,
					&pm8921_gpios_ptr[i].config);
		if (rc) {
...
static struct msm_camera_sensor_flash_src msm_flash_src = {
	...
	._fsrc.ext_driver_src.led_en = /*GPIO_CAM_GP_LED_EN1*/0,
	._fsrc.ext_driver_src.led_flash_en = /*GPIO_CAM_GP_LED_EN2*/0,
...
#define MSM_FB_PRIM_BUF_SIZE (720 * 1280 * 4 * 3)
#else
/* prim = 608 x 1024 x 4(bpp) x 2(pages) */
#define MSM_FB_PRIM_BUF_SIZE (720 * 1280 * 4 * 2)
#endif

/*
 * TODO.. do we need to change the writeback size to match with panel
 * resolution
 */
#ifdef CONFIG_FB_MSM_MIPI_DSI
#define MIPI_DSI_WRITEBACK_SIZE (720 * 1280 * 4 * 2)
#else
...
#warning MOT_MERGE_WARNING mipi_dsi_cdp_panel_power conflicts with QCOM 1.0.20
...
static struct mmc_platform_data msm8960_sdc3_data = {
	...
	/* PMIC 22 for Teufel 1, PMIC 20 For Teufel 2, Vanquish and Dinara */
	.status_gpio	= PM8921_GPIO_PM_TO_SYS(20),
	.status_irq	= PM8921_GPIO_IRQ(PM8921_IRQ_BASE, 20),
...
static void __init msm8960_init_mmc(void)
{
	...
	if (machine_is_teufel()) {
		pr_err("machine is teufel\n");
		msm8960_sdc3_data.status_gpio	= PM8921_GPIO_PM_TO_SYS(22);
		msm8960_sdc3_data.status_irq = PM8921_GPIO_IRQ(PM8921_IRQ_BASE, 22);
	}
...
static char *usb_functions_factory_adb[] = {
	"usbnet",
	"adb",
};

static char *usb_functions_bptools[] = {
	"diag",
	"modem",
	"nmea",
	"usbnet",
...
static char *usb_functions_bptools_adb[] = {
	"diag",
	"modem",
	"nmea",
	"usbnet",
...
static char *usb_functions_bptools_rndis[] = {
#ifdef CONFIG_USB_ANDROID_RNDIS
	"rndis",
#endif
	"diag",
...
static char *usb_functions_bptools_rndis_adb[] = {
#ifdef CONFIG_USB_ANDROID_RNDIS
	"rndis",
#endif
	"diag",
...
struct android_usb_product usb_products_factory[] = {
	{
		.product_id	= 0x2e2c,
		.num_functions	= ARRAY_SIZE(usb_functions_default_adb),
	...
	},
	{
		.product_id	= 0x2e2d,
		.num_functions	= ARRAY_SIZE(usb_functions_default_adb),
		.functions	= usb_functions_factory_adb,
	},
};

struct android_usb_product usb_products_bptools[] = {
	{
		.product_id	= 0x2e28,
		.num_functions	= ARRAY_SIZE(usb_functions_bptools),
		.functions	= usb_functions_bptools,
	},
	{
		.product_id	= 0x2e29,
		.num_functions	= ARRAY_SIZE(usb_functions_bptools_adb),
		.functions	= usb_functions_bptools_adb,
	},
	{
		.product_id	= 0x2e2a,
		.num_functions	= ARRAY_SIZE(usb_functions_bptools_rndis),
		.functions	= usb_functions_bptools_rndis,
	},
	{
		.product_id	= 0x2e2b,
		.num_functions	= ARRAY_SIZE(usb_functions_bptools_rndis_adb),
		.functions	= usb_functions_bptools_rndis_adb,
	},
};

static struct android_usb_platform_data android_usb_pdata_default = {
	.vendor_id	= 0x22b8,
	.product_id	= 0x2e2d,
	.version	= 0x0100,
	.product_name		= "Motorola 8960",
	.manufacturer_name	= "Motorola Mobility",
	.num_products = ARRAY_SIZE(usb_products_default),
	.products = usb_products_default,
	.num_functions = ARRAY_SIZE(usb_functions_default_adb),
	.functions = usb_functions_default_adb,
	.serial_number = "1234567890ABCDEF",
};

static struct android_usb_platform_data android_usb_pdata_factory = {
	.vendor_id	= 0x22b8,
	.product_id	= 0x2e2d,
	.version	= 0x0100,
	.product_name		= "Motorola 8960",
	.manufacturer_name	= "Motorola Mobility",
	.num_products = ARRAY_SIZE(usb_products_factory),
	.products = usb_products_factory,
	.num_functions = ARRAY_SIZE(usb_functions_factory_adb),
	.functions = usb_functions_factory_adb,
	.serial_number = "1234567890ABCDEF",
};

static struct android_usb_platform_data android_usb_pdata_bptools = {
	.vendor_id	= 0x22b8,
	.product_id	= 0x2e29,
	.version	= 0x0100,
	.product_name		= "Motorola 8960",
	.manufacturer_name	= "Motorola Mobility",
	.num_products = ARRAY_SIZE(usb_products_bptools),
	.products = usb_products_bptools,
	.num_functions = ARRAY_SIZE(usb_functions_bptools_rndis_adb),
	.functions = usb_functions_bptools_rndis_adb,
	.serial_number = "1234567890ABCDEF",
};
...
static struct pm8921_charger_platform_data pm8921_chg_pdata __devinitdata = {
	...
	.resume_voltage		= 4100,
	.term_current		= 100,
	.cool_temp		= 0, /* XXX need to determine proper value */
	.warm_temp		= 0, /* XXX need to determine proper value */
	.temp_check_period	= 1,
...
static void __init register_i2c_devices(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(msm8960_i2c_devices); ++i) {
		if (is_smd() && !strncmp("lm3532",
				msm8960_i2c_devices[i].info->type, strlen("lm3532"))) {
			pr_info("%s: SMD display detected. Skipping lm3532\n", __func__);
			continue;
		}
		if (is_smd() &&
			!strncmp("cyttsp3",
				 msm8960_i2c_devices[i].info->type,
				 strlen("cyttsp3"))) {
			continue;
		}
		if (!is_smd() &&
		...
		if (!mach_mask || (msm8960_i2c_devices[i].machs & mach_mask)) {
...
static void __init msm8960_mmi_init(void)
{
	...
	/* Set Motorola keymap*/
	pm8921_platform_data.keypad_pdata = &mot_keypad_data;
