...
static void melfas_ts_get_data(struct melfas_ts_data *ts)
{
	...
	if(read_num > 0) {
		...
		for(i=0; i<read_num; i=i+6)
		{
			...
			mtouch_info[finger_id].xpos= (uint16_t)(buf[i+1] & 0x0F) << 8 | buf[i+2];
			mtouch_info[finger_id].ypos= (uint16_t)(buf[i+1] & 0xF0) << 4 | buf[i+3];
		...
		for(i=0; i<MELFAS_MAX_TOUCH; i++)
		{
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION_X, mtouch_info[i].xpos);
			input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, mtouch_info[i].ypos);
			input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, mtouch_info[i].strength );
			input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, mtouch_info[i].width);
			input_mt_sync(ts->input_dev);

			if (tsdebug >= TS_DBG_LVL_1)
				pr_info("Touch ID: %d, State : %d, x: %d, y: %d, z: %d w: %d\n",
					 i, (mtouch_info[i].strength>0), mtouch_info[i].xpos,
					 mtouch_info[i].ypos, mtouch_info[i].strength, mtouch_info[i].width);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case TS_DBG_LVL_3:
		...
		break;
	default:
		pr_err("%s: Invalid option, 1/0 = Intr enable/disable setting=%d\n",
			__func__, (int)value);
		break;
	}
				 ...
				 const struct i2c_device_id *id)
{
	...
	pr_info("%s:\n Panel Version: %x, HW Revision: %x\n HW Comp Grp: %x,"
		 " Core FW ver: %x\n Private FW ver: %x, Public FW ver: %x \n", \
		 __func__, ts->version_info.panel_ver, ts->version_info.hw_ver, \
	...
	if (ret) {
		...
		ret = -ENODEV;
		goto err_create_intr_device_failed;
	}
...
err_create_intr_device_failed:
	pr_err("%s: device creation for intr failed \n", __func__);
